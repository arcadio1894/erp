...
@section('page-title')
    <h5 class="card-title">Editar porcentaje {{$percentageWorker->name}}</h5>
@endsection
...
@section('content')
    <form id="formEdit" class="form-horizontal" data-url="{{ route('percentageWorker.update') }}" enctype="multipart/form-data">
        @csrf
        <input type="hidden" class="form-control" name="percentage_id" value="{{$percentageWorker->id}}">

        <div class="form-group row">
            <div class="col-md-6">
                <label for="inputEmail3" class="col-12 col-form-label">Nombre <span class="right badge badge-danger">(*)</span></label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="name" placeholder="Ejm: Porcentaje" value="{{$percentageWorker->name}}">
                </div>
            </div>
        </div>
        ...
        <div class="form-group row">
            <div class="col-md-6">
                ...
                <div class="col-sm-10">
                    <input type="number" class="form-control" name="value" min="0" step="0.01" value="{{$percentageWorker->value}}">
                </div>
            </div>
        </div>
