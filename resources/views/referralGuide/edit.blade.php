...
@section('page-title')
    <h5 class="card-title">Editar guía de remisión {{ $arrayGuide[0]["code"] }}</h5>
@endsection
...
@section('content')
    ...
    <div class="form-group row">
        <input type="hidden" id="guide_id" value="{{ $arrayGuide[0]["id"] }}">
        <div class="col-md-6">
            <div class="col-md-12" id="sandbox-container">
                ...
                <div class="col-md-12 input-daterange" id="datepicker">
                    <input type="text" class="form-control date-range-filter" id="date_transfer" name="date_transfer" value="{{ $arrayGuide[0]["date_transfer"] }}">
                </div>
            </div>
            <div class="col-md-12">
                ...
                <div class="col-sm-12">
                    <select id="reason_id" class="form-control select2" name="reason_id" style="width: 100%;">
                        ...
                        @foreach( $reasons as $reason )
                            <option value="{{ $reason->id }}" {{ ($reason->id == $arrayGuide[0]["reason_transfer_id"]) ? 'selected':'' }}>{{ $reason->description }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="row ml-1 mr-1">
                    ...
                    <div class="col-md-4 mb-2">
                        <select id="destination" class="form-control select2" style="width: 100%;">
                            <option></option>
                            <option value="Cliente" {{ ( $arrayGuide[0]["destination"] == 'Cliente' ) ? 'selected':'' }}>Cliente</option>
                            <option value="Proveedor" {{ ( $arrayGuide[0]["destination"] == 'Proveedor' ) ? 'selected':'' }}>Proveedor</option>
                            <option value="Otros" {{ ( $arrayGuide[0]["destination"] == 'Otros' ) ? 'selected':'' }}>Otros</option>
                        </select>
                    </div>
                </div>
                <div class="col-sm-12">
                    <div id="div_customer_id" style="display: {{ ( $arrayGuide[0]["destination"] == 'Cliente' ) ? '':'none' }};"  >
                        <select id="customer_id" class="form-control select2" name="customer_id" style="width: 100%">
                            <option></option>
                            @foreach( $customers as $customer )
                                <option value="{{ $customer->id }}" {{ ( $arrayGuide[0]["customer_id"] == $customer->id ) ? 'selected':'' }} data-ruc="{{ $customer->RUC }}" data-address="{{ $customer->address }}">{{ $customer->business_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div id="div_supplier_id" style="display: {{ ( $arrayGuide[0]["destination"] == 'Proveedor' ) ? '':'none' }};" >
                        <select id="supplier_id" class="form-control select2" name="supplier_id" style="width: 100%">
                            <option></option>
                            @foreach( $suppliers as $supplier )
                                <option value="{{ $supplier->id }}" {{ ( $arrayGuide[0]["supplier_id"] == $supplier->id ) ? 'selected':'' }} data-ruc="{{ $supplier->RUC }}" data-address="{{ $supplier->address }}">{{ $supplier->business_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div id="div_receiver" style="display: {{ ( $arrayGuide[0]["destination"] == 'Otros' ) ? '':'none' }};" >
                        <input type="text" class="form-control" id="receiver" value="{{ $arrayGuide[0]["destination"] }}">
                    </div>
        ...
        <div class="col-md-6">
            <div class="col-md-12">
                ...
                <div class="col-sm-12">
                    <textarea id="puntoLlegada" class="form-control" rows="3">{{ $arrayGuide[0]["punto_llegada"] }}</textarea>
                </div>
            </div>
            <div class="col-md-12">
                <label for="document" class="col-md-12 col-form-label">RUC/DNI <span class="right badge badge-danger">(*)</span></label>
                <div class="col-sm-12">
                    <input type="text" class="form-control" id="document" value="{{ $arrayGuide[0]["documento"] }}">
                </div>
            </div>
        </div>
    </div>
                                ...
                                <table class="table table-head-fixed text-nowrap">
                                    ...
                                    @foreach ($arrayGuide[0]['details'] as $detail)
                                        <tr data-row_selected>
                                            ...
                                            <td>
                                                <button type="button" data-delete="{{ $detail['id'] }}" data-type="{{ $detail['type'] }}" class="btn btn-danger"><i class="fas fa-trash"></i></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
    ...
    <div class="col-md-6">
        ...
        <div class="col-sm-6">
            <input type="text" id="placa" class="form-control" value="{{ $arrayGuide[0]['vehiculo'] }}">
        </div>
    </div>
    ...
    <div class="col-md-6">
        ...
        <div class="col-sm-10">
            <input type="text" id="driver" class="form-control" value="{{ $arrayGuide[0]['driver'] }}">
        </div>
        ...
        <div class="col-sm-6">
            <input type="text" id="driver_licence" class="form-control" value="{{ $arrayGuide[0]['driver_licence'] }}">
        </div>
        ...
        <div class="col-sm-6">
            <select id="responsible_id" class="form-control select2" name="customer_id" style="width: 100%">
                ...
                @foreach( $shippingManagers as $shippingManager )
                    <option value="{{ $shippingManager->id }}" {{ ($shippingManager->id == $arrayGuide[0]['responsible']) ? 'selected': '' }}>{{ $shippingManager->user->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="text-center">
        <button type="button" id="btn-submit" data-url="{{ route('referral.guide.update', $arrayGuide[0]['id']) }}" class="btn btn-outline-success">Guardar cambios</button>
...
@section('scripts')
    ...
    <script src="{{ asset('js/referralGuide/edit.js') }}?v={{ time() }}"></script>
@endsection
