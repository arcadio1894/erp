...
<table id="table">
    <thead>
        <tr>
            ...
            <th width="180px" style="background-color: #7A8DC5; font-size: 14px; word-wrap: break-word">Bono</th>
    ...
    @for ( $i = 0; $i<count($bonuses); $i++ )
        @if ( ($i+1) % 2 == 0)
            <tr>
                <th width="250px">{{ $bonuses[$i]['trabajador'] }}</th>
                <th width="90px">{{ $bonuses[$i]['fecha'] }}</th>
                <th width="100px" style="word-wrap: break-word">{{ $bonuses[$i]['week'] }}</th>
                <th width="200px" style="word-wrap: break-word">{{ $bonuses[$i]['reason'] }}</th>
                <th width="80px" style="text-align: right">{{ $bonuses[$i]['total'] }}</th>
            </tr>
        @else
            <tr>
                <th width="250px">{{ $bonuses[$i]['trabajador'] }}</th>
