...
<main>
    ...
    <table>
        ...
        @foreach( $quote->equipments as $equipment )
            @if ( $quote->state_decimals == 1 )
                ...
                <tr>
                    <td class="desc">{{ $consumable->material->full_name }}</td>
                    <td class="qty">{{ $equipment->quantity }}</td>
                    <td class="unit">{{ $quote->currency_invoice }} {{ number_format($consumable->valor_unitario,0) }}</td>
                    <td class="unit">{{ $quote->currency_invoice }} {{ number_format($consumable->price, 0) }}</td>
                    <td class="total">{{ $quote->currency_invoice }} {{ number_format($consumable->total, 0) }}</td>
                </tr>
                @endforeach
            @else
                @foreach( $equipment->consumables as $consumable )
                <tr>
                    <td class="desc">{{ $consumable->material->full_name }}</td>
                    <td class="qty">{{ $equipment->quantity }}</td>
                    <td class="unit">{{ $quote->currency_invoice }} {{ number_format($consumable->valor_unitario,2) }}</td>
                    <td class="unit">{{ $quote->currency_invoice }} {{ number_format($consumable->price, 2) }}</td>
                    <td class="total">{{ $quote->currency_invoice }} {{ number_format($consumable->total, 2) }}</td>
                </tr>
                @endforeach
            @endif
    ...
    <table id="sumary">
        ...
        <tr>
            ...
            @if ( $quote->state_decimals == 1 )
                <td class="total">{{ $quote->currency_invoice }} {{ number_format( $quote->descuento, 0) }}.00</td>
            @else
                <td class="total">{{ $quote->currency_invoice }} {{ number_format( $quote->descuento, 2) }}</td>
            @endif
        </tr>
        <tr>
            <td class=""></td>
            <td class=""></td>
            <td class="qty">GRAVADA:</td>
            @if ( $quote->state_decimals == 1 )
                <td class="total">{{ $quote->currency_invoice }} {{ number_format( ($quote->gravada), 0) }}.00</td>
            @else
                <td class="total">{{ $quote->currency_invoice }} {{ number_format( ($quote->gravada), 2) }}</td>
            @endif
        </tr>
        <tr>
            <td class=""></td>
            <td class=""></td>
            <td class="qty">IGV {{ $igv }}%:</td>
            @if ( $quote->state_decimals == 1 )
                <td class="total">{{ $quote->currency_invoice }} {{ number_format( $quote->igv_total, 0) }}.00</td>
            @else
                <td class="total">{{ $quote->currency_invoice }} {{ number_format( $quote->igv_total, 2) }}</td>
            @endif
        </tr>
        <tr>
            <td class=""></td>
            ...
            @if ( $quote->state_decimals == 1 )
                <td class="total">{{ $quote->currency_invoice }} {{ number_format( $quote->total_importe, 0) }}.00</td>
            @else
                <td class="total">{{ $quote->currency_invoice }} {{ number_format( $quote->total_importe, 2) }}</td>
            @endif
        </tr>
        {{--<tr>
            <td class="desc"></td>
