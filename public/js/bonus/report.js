...
function downloadReportExpense() {
    ...
    var url = "/dashboard/descargar/excel/bonos/especiales?" + $.param(query);
...
function generateReportExpense() {
    ...
    if ( type == 1 )
    {
        ...
        $.get( "/dashboard/generate/report/bonus?" + $.param(query), function( data ) {
            //console.log( data );
        }).done(function(data) {
            console.log("Semanal");
            agregarFilas(data.bonuses);
            $('#report-expense').show();
            //console.log( data );
        }).fail(function(data) {
        ...
        $.get( "/dashboard/generate/report/bonus?" + $.param(query2), function( data ) {
            //console.log( data );
        }).done(function(data) {
            console.log("Mensual");
            agregarFilas(data.bonuses);
...
function agregarFilas(datos) {
    ...
    for (var i = 0; i < datos.length; i++) {
        ...
        var nuevaFila = '<tr>' +
            ...
            '<td>' + fila.reason + '</td>' +
            '<td>' + moment(fila.date).format('DD-MM-YYYY') + '</td>' +
            '<td>' + fila.amount + '</td>' +
            '</tr>';
