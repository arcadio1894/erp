    ...
    $('#dynamic-table').DataTable( {
        ...
        "aoColumns": [
            ...
            { data: null,
                ...
                {
                    var text = '';
                    if ( $.inArray('edit_refund', $permissions) !== -1 ) {
                        text = text + '<a href="'+document.location.origin+ '/dashboard/editar/reembolso/'+item.id+
                            '" class="btn btn-outline-warning btn-sm" data-toggle="tooltip" data-placement="top" title="Editar reembolso"><i class="fa fa-pen"></i></a>';
                    }
                    if ( $.inArray('destroy_refund', $permissions) !== -1 ) {
                        text = text + ' <button data-delete="'+item.id+'" data-reason="'+ item.reason +'" data-date="'+moment(item.date).format('DD/MM/YYYY')+'" '+
                            ' class="btn btn-outline-danger btn-sm" data-toggle="tooltip" data-placement="top" title="Eliminar reembolso"><i class="fa fa-trash"></i></button>';
                    }
                    return text;
...
function openModalDelete() {
    var refund_id = $(this).data('delete');
    var date = $(this).data('date');
    var reason = $(this).data('reason');

    var texto = reason + ' - ' + date;

    $modalDelete.find('[id=refund_id]').val(refund_id);
