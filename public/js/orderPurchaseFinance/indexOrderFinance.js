$(document).ready(function () {
    ...
    $('#dynamic-table').DataTable( {
        ajax: {
            url: "/dashboard/all/order/finance",
            dataSrc: 'data'
        },
        bAutoWidth: false,
        "aoColumns": [
            ...
            { data: null,
                ...
                "render": function (item)
                {
                    ...
                    if ( $.inArray('list_orderPurchaseFinance', $permissions) !== -1 ) {
                        text = text + '<a target="_blank" href="'+document.location.origin+ '/dashboard/imprimir/orden/compra/finanza/'+item.id+'" class="btn btn-outline-info btn-sm" data-toggle="tooltip" data-placement="top" title="Imprimir Orden"><i class="fa fa-print"></i> </a>  ';
                    }
                    if ( $.inArray('list_orderPurchaseFinance', $permissions) !== -1 ) {
                        text = text + '<a target="_blank" href="'+document.location.origin+ '/dashboard/ver/orden/compra/finanzas/'+item.id+
                            '" class="btn btn-outline-primary btn-sm" data-toggle="tooltip" data-placement="top" title="Ver Orden"><i class="fa fa-eye"></i></a> ';
                    }
                    if ( $.inArray('update_orderPurchaseFinance', $permissions) !== -1 ) {
                        text = text + '<a href="'+document.location.origin+ '/dashboard/editar/orden/compra/finanza/'+item.id+
                    ...
                    /*if (item.regularize == 'nr')
                    {
                        if ( $.inArray('regularize_orderService', $permissions) !== -1 ) {
                            text = text + '<a href="'+document.location.origin+ '/dashboard/ingresar/orden/servicio/'+item.id+
                                '" class="btn btn-outline-success btn-sm" data-toggle="tooltip" data-placement="top" title="Crear Ingreso"><i class="fa fa-share"></i></a> ';
                        }
                    }*/
...
function cancelOrden() {
    ...
    $.confirm({
        ...
        animation: 'zoom',
        type: 'red',
        title: '¿Está seguro de eliminar esta orden de compra de finanza?',
        content: 'Luego puede restaurarla.',
        buttons: {
            confirm: {
                text: 'CONFIRMAR',
                action: function (e) {
                    $.ajax({
                        url: '/dashboard/destroy/order/purchase/finance/'+order_id,
                        method: 'POST',
                        ...
                        success: function (data) {
                            console.log(data);
                            $.alert("Orden de compra de finanzas anulada.");
                            setTimeout( function () {
                                location.reload();
                            }, 2000 )
                        },
