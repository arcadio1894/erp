$(document).ready(function () {
    ...
    $('#dynamic-table').DataTable( {
        ...
        "aoColumns": [
            ...
            { data: null,
                ...
                {
                    var text = '';
                    if ( $.inArray('update_unitMeasure', $permissions) !== -1 ) {
                        text = text + '<a href="'+document.location.origin+ '/dashboard/editar/unidad/'+item.id+
                            '" class="btn btn-outline-warning btn-sm"><i class="fa fa-pen"></i></a>';
                    }
                    if ( $.inArray('destroy_unitMeasure', $permissions) !== -1 ) {
    ...
    $formDelete.on('submit', destroyUnitMeasure);
        ...
        $.confirm({
            ...
            buttons: {
                confirmar: {
                    ...
                    action: function () {
                        $.ajax({
                            url: '/dashboard/unitmeasure/delete-multiple',
                            type: 'POST',
                            data: {
                                ids: ids,
...
function openModalDelete() {
    var unitMeasure_id = $(this).data('delete');
    var name = $(this).data('name');

    $modalDelete.find('[id=unitMeasure_id]').val(unitMeasure_id);
    $modalDelete.find('[id=name]').html(name);

    $modalDelete.modal('show');
}

function destroyUnitMeasure() {
